...
- Admin deploy status API/UI (`GET /api/admin/deploy` and `/admin` panel) backed by deployhook status file updates.
...
- Added deployment docs for same-IP operation with existing nginx (`/docs/same-ip-nginx` and `deploy/NGINX_SAME_IP.md`).
...
- Embedded docs deep links now resolve correctly for clean URLs (for example `/docs/command-http`).
