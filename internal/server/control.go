...
func parseHeaderNameList(field string, values []string) ([]string, error) {
	...
	for _, v := range values {
		name, err := control.NormalizeHeaderName(field, v)
		if err != nil {
			return nil, err
		}
		out = append(out, name)
...
func parseHeaderKVList(field string, values []control.HeaderKV) ([]headerKV, error) {
	...
	for _, kv := range values {
		name, err := control.NormalizeHeaderName(field, kv.Name)
		if err != nil {
			return nil, err
		}

		val, err := control.ValidateHeaderValue(field, kv.Name, kv.Value)
		if err != nil {
			return nil, err
		}
