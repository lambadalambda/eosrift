...
func controlHandler(cfg Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewDecoder(ctrlStream).Decode(&req); err != nil {
			_ = writeControlError(ctrlStream, "invalid request")
			_ = ctrlStream.Close()
			return
		}

		if req.Type != "tcp" {
			_ = writeControlError(ctrlStream, "unsupported tunnel type")
			_ = ctrlStream.Close()
			return
		}
		...
		if err != nil {
			_ = writeControlError(ctrlStream, err.Error())
			_ = ctrlStream.Close()
			return
		}
		defer ln.Close()
...
func writeControlError(w io.Writer, msg string) error {
